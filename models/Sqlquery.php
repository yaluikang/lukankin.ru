...
class Sqlquery extends Query
{
    ...
    public function getMovieBySearch( $search )
    {
        ...
        $movies = $this->select("movies_id, movies_name, movies_url_poster, movies_date")->FROM("movies")->where(['like', 'movies_name', $search])->createCommand()->sql;
    ...
    public function findMoviesByGenre( $genre, $q )
    {
        ...
        $this->join = null;
        $movies = $this->select("movies_id, movies_name, movies_url_poster, movies_date")->FROM("movies")->where(['movies_id' => $ids ])->andWhere(['movies_qualifier' => $this->moviesQualifier])->offset( $borders['offset'] )->limit( $borders['limit'] )->all();
        $moviesWithGenres = $this->addGenresForMovies($movies);
        echo json_encode( $moviesWithGenres,JSON_UNESCAPED_UNICODE );
    }
