    ...
    <div class="row margin MyMovies">
        <nav class="navbar navbar-expand-lg navbar-light col-sm-12 col-md-12 col-auto" id="nav">
            ...
            <?php
                ...
                {
                    echo '<a href="/login">Вход</a>/<a href="/join">Регистрация</a>';
                } else {
                    $name = Yii::$app->user->getIdentity()->name;
                    $urlLogout = Url::to(['site/logout']);
                    echo "<a href='/account'>Личный кабинет(".$name.")</a>/<a href='".$urlLogout."' style='margin-left: 10px;' id='logout'>Выход</a>";
                }
            ?>
        </nav>
    </div>
...
<?php
if( !preg_match('/\/account/', $_SERVER['REQUEST_URI'], $matches, PREG_OFFSET_CAPTURE) || !preg_match('/\/login/', $_SERVER['REQUEST_URI'], $matches2, PREG_OFFSET_CAPTURE) || !preg_match('/\/join/', $_SERVER['REQUEST_URI'], $matches3, PREG_OFFSET_CAPTURE))
{
